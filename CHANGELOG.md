...
### CLI連携ロジックのリファクタリングと互換性向上

**内部実装の変更と改善:**

- `_executeToolRaw` のリクエスト処理方式の変更
  - `response.body.getReader()` を用いたストリーム逐次受信から、最新の CLI (1.0.23 以降の `client.js`) の挙動に合わせ、`response.text()` による一括受信＋改行分割パース方式に移行。
  - `ReadableStream.getReader()` に依存しないため、古いブラウザやエッジ環境（Cloudflare Workers 等）での環境互換性と安定性が向上。
- プレーンな JSON レスポンスへの対応
  - サーバーが `status` や `version` フィールドを持たない通常のプレーンな JSON を返してきた場合でも、エラーと判定せず最終結果として正しくフォールバックできる判定条件を追加。

**テスト:**

- モックテストスクリプト (`test2.js`) の新規追加
  - クレジットやAPI呼び出し枠を消費せずに、あらゆるレスポンスパターンに対するパースロジックの動作検証をローカル環境で安全かつ瞬時に行えるテストを追加。
