...
- **便利メソッド** — Web検索、画像・マルチメディア解析、AI Drive操作などを簡潔に呼び出せる
...
**引数:**
- `urls` (`string[]`) — メディアURLの配列
- `instruction` (`string`) — 解析の指示
- `options.analyze_type` (`string`) — 解析の種類（任意）
...
ローカルのメディアファイルを解析します（内部でアップロード → 解析）。

```javascript
const blob = new Blob([fs.readFileSync('video.mp4')], { type: 'video/mp4' });
const { results } = await client.analyzeLocalMultimedia(blob, '何が映っていますか？');
```

**引数:**
- `media` (`File|Blob`) — メディアデータ
- `instruction` (`string`) — 解析の指示
